...
/*
IRModule pass to generate C source code.
parms:
  - mod :经过优化的IRModule
  - target : 目标设备信息
*/
runtime::Module BuildCHost(IRModule mod, Target target) {
  ...
  bool output_ssa = false;          // 是否生成SSA（静态单赋值）形式的C代码
  bool emit_asserts = false;        // 是否在生成的代码中包含断言（用于调试）
  bool emit_fwd_func_decl = true;   // 是否生成函数前向声明（确保函数调用顺序不影响编译）
  ...
  // 初始化CodeGenCHost实例
  // 负责将PrimFunc（TIR 原始函数）转换为 C 代码的核心类
  CodeGenCHost cg;
  ...
  for (auto kv : mod->functions) {
    ...
    if (is_aot_executor_fn) {
      aot_executor_fn = Downcast<PrimFunc>(kv.second);// AOT执行器函数（如__tvm_main__）
      continue;
    }
    funcs.push_back(kv); // 普通函数
  }

  // Sort functions 按函数名排序，确保代码生成顺序一致
  ...
  if (aot_executor_fn.defined() && runtime->name == relay::kTvmRuntimeCpp) {
    cg.InitGlobalContext();// 初始化C++运行时全局上下文（如内存管理器）
  }
  ...
  // 封装为CSourceModule
  //包括： C 代码 、运行时依赖信息（如需要链接libtvm_runtime库）
}
