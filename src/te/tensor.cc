 ...
 * \file tensor.cc
 */
#include <tvm/runtime/registry.h> // TVM 运行时注册机制（暴露C++函数给Python）
#include <tvm/te/operation.h>     // 张量操作（Operation）相关定义
#include <tvm/te/tensor.h>        // 张量（Tensor）类定义
#include <tvm/te/tensor_intrin.h> // 张量内联函数（TensorIntrin）相关定义

#include <memory>                 // 智能指针（用于管理对象生命周期）
...
inline PrimExpr Tensor::IndexTensor(Array<PrimExpr> indices, bool support_negative_indices) const {
  Array<PrimExpr> shape = (*this)->shape;
   // 1. 维度检查：确保索引数量与张量维度一致
  if (shape.size() != 0) {
    ICHECK_EQ(shape.size(), indices.size())
        << "Tensor dimension mismatch in read "
        << "ndim = " << ndim() << ", indices.size=" << indices.size();
  }
  // 2. 负索引支持：若开启，将负索引转为正索引（如 -1 → shape[i]-1）
