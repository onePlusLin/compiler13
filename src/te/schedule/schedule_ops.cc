 ...
 * \file schedule_ops.cc
  ScheduleOps 函数的底层 C++ 实现，
  核心作用是将开发者定义的 Schedule（调度）—— 
  包含循环优化、硬件绑定、内存策略等抽象指令 ——
  转换为具体的 TVM 中间表示（Stmt）
 */
#include <tvm/runtime/registry.h>  // TVM 函数注册
#include <tvm/te/operation.h>      // 计算操作（Operation）定义
#include <tvm/te/schedule_pass.h>  // SchedulePass 接口声明
#include <tvm/tir/analysis.h>      // TIR 分析工具（如副作用分析）
#include <tvm/tir/expr.h>          // TIR 表达式（如变量、常量）
#include <tvm/tir/stmt_functor.h>  // Stmt 遍历与修改工具（StmtMutator）

#include <unordered_map>
#include <unordered_set>
#include <utility>

#include "../../tir/transforms/ir_utils.h"  // TIR 工具函数
#include "../operation/op_utils.h"          // Operation 辅助函数
#include "graph.h"                          // 计算图相关工具
...
Stmt WrapLayoutTransformationAttrs(const Stage& stage, Stmt body) {
  // 1. 若阶段有布局转换（如 transform_layout 调用），为每个输出张量添加注解
  if (stage->layout_transforms.size()) {
    for (int i = 0; i < stage->op->num_outputs(); i++) {
      body = AttrStmt(
        Array<ObjectRef>{stage->op.output(i), stage->layout_transforms},  // 注解参数：张量+布局转换规则
        tir::attr::layout_transforms,  // 注解类型：布局转换
        1,  // 注解值（占位，无实际意义）
        body  // 被注解的语句（计算逻辑）
      );
    }
  ...
  // 2. 若阶段有轴分隔符（如 set_axis_separators 调用），为每个输出张量添加注解
  if (stage->axis_separators.size()) {
    for (int i = 0; i < stage->op->num_outputs(); i++) {
      body = AttrStmt(
        Array<ObjectRef>{stage->op.output(i), stage->axis_separators},  // 注解参数：张量+轴分隔符
        tir::attr::axis_separators,  // 注解类型：轴分隔符
        1,
        body
      );
    }
...
/*
生成阶段计算流水线
根据阶段（Stage）的配置（如双缓冲、内存范围），生成包含 “计算逻辑 + 内存分配 + 优化注解” 的完整 Stmt 流水线。
*/
