...
IRModule SequentialNode::operator()(IRModule mod, const PassContext& pass_ctx) const {
  // 1. 遍历 Sequential 中管理的所有 Pass（按列表顺序）
  for (const Pass& pass : passes) {
    // 2. 打印调试日志：当前正在执行的 Pass 名称（VLOG(0) 表示基础日志级别）
    VLOG(0) << "Running pass " << pass->Info()->name;
    
    // 3. 合法性检查：确保当前 Pass 已正确定义（避免空指针）
    ICHECK(pass.defined()) << "Found undefined pass for optimization.";
    
    // 4. 获取当前 Pass 的元数据（PassInfo：包含名称、优化级别、依赖列表等）
    const PassInfo& pass_info = pass->Info();
    
    // 5. 检查当前 Pass 是否被上下文（PassContext）启用
    if (!pass_ctx.PassEnabled(pass_info)) {
      VLOG(0) << "skipping disabled pass '" << pass_info->name << "'";
      continue;  // 若被禁用，跳过当前 Pass，执行下一个
    }
    
    // 6. 解析并执行当前 Pass 的依赖 Pass
    for (const auto& required_pass_name : pass_info->required) {
      // 6.1 通过依赖 Pass 的名称获取对应的 Pass 实例（GetPass 是全局获取 Pass 的函数）
      // 6.2 执行依赖 Pass，将结果模块传递给下一个步骤（std::move 避免拷贝开销）
      mod = GetPass(required_pass_name)(std::move(mod), pass_ctx);
    }
    
    // 7. 执行当前 Pass，将优化后的模块更新为下一个 Pass 的输入
    mod = pass(std::move(mod), pass_ctx);
  }
  // 8. 所有 Pass 执行完毕，返回最终优化后的 IRModule
  return mod;
}
