    ...
    namespace transform {
      ...
      namespace {
        ...
        class TargetHookVisitor : public MixedModeVisitor {
          ...
          void MaybeAddPassForFunction(const FunctionNode* function_node) {
            ...
            std::string kind_name = opt_compiler.value();//ethosu
        ...
        }  // namespace 这里似乎只会调用ethosu的pass
      //  CompilationConfig 参数，包含了所有可用的目标设备配置信息，并返回一个 Pass 对象。
      Pass RelayToTIRTargetHook(CompilationConfig config) {
        auto pass_func = [config = std::move(config)](IRModule mod, const PassContext& pass_ctx) {
          ...
          std::vector<CustomPass> custom_passes = target_hook_visitor.Visit();// 利用这个找到对应的pass的
          for (const auto& custom_pass : custom_passes) {
            if (custom_pass.opt_target.defined()) {
              ...
              VLOG(0) << "Invoking custom pass for target kind '" << custom_pass.target_kind_name << "'";// Invoking custom pass for target kind 'ethos-u'
              mod = custom_pass.pass(mod);
              }
            }
