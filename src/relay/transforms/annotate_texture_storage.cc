...
namespace tvm {
  namespace relay {
    ...
    Map<Expr, Map<Expr, Array<String>>> CollectStorageInfo(const Expr& expr) {
      //收集表达式中涉及的所有虚拟设备类型，例如 "opencl", "cuda", "llvm" 等
      ...
      //根据设备类型构建目标函数名称前缀。例如，如果设备类型是 "opencl.adreno"，那么前缀会是 "relay.backend.opencl.adreno"。
      ...
      //在 TVM 的运行时注册表中查找并调用特定于目标设备的存储信息收集函数。
    ...
    Expr AnnotateMemoryScopeExpr(const Expr& expr, const IRModule& mod) {
      auto storage_scope = CollectStorageInfo(expr); // 收集表达式的存储信息
      if (storage_scope.size()) {
        return RewriteVDStorageScopes(storage_scope).Rewrite(expr);//  创建一个表达式重写器实例，传入之前收集的存储信息 注解内存作用域
        }
      else {
        return expr;
        }
