...
namespace tvm {
  namespace relay {
    ...
    /*
    *  VisitExpr 是修改一个表达式 'expr' 的入口函数
    */
    Expr MixedModeMutator::VisitExpr(const Expr& expr) {
      // 1. 定义第一个 lambda 函数 fcheck_visited。
      //    检查一个节点是否被访问过。
      //    CheckVisited 方法”。
      auto fcheck_visited = [this](const Expr& expr) { return this->CheckVisited(expr); };

      // 2. 定义第二个 lambda 函数 fvisit_leaf。
      //    这是另一张更重要的“指令卡”，上面写着：“要处理一个独立的零件（叶子节点），
      //   VisitLeaf 方法”。
      //    （我们已经知道 VisitLeaf 内部会进一步调用 DispatchVisitExpr）。
      auto fvisit_leaf = [this](const Expr& expr) { return this->VisitLeaf(expr); };

      // 如果已经有记录，直接返回缓存：变异的结果在映射中记忆并重复使用，以便数据流上的局部转换保留图形结构。
      if (memo_.count(expr)) {
        return memo_[expr];
        }
      else {
    ...
    Expr ExprMutator::VisitExpr(const Expr& expr) {
      auto it = this->memo_.find(expr);//检查是否访问
      if (it != this->memo_.end()) {
        return it->second;
        }
      else {// 关键：调用 ExprFunctor 的分发机制
        Expr new_expr = ExprFunctor::VisitExpr(expr);// 这里进行类型分发
        memo_[expr] = new_expr;
        return new_expr;
        }
      }
