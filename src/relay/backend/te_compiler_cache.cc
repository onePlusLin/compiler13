    ...
    namespace tec {
      ...
      class LowerToTECompute : public backend::MemoizedExprTranslator<Array<te::Tensor>> {
        ...
        Array<te::Tensor> Lower(const Function& relay_func) {
          for (Var param : relay_func->params) {//给每个输入变量创建placeholder张量
            Array<tvm::te::Tensor> inputs;
            for (const auto& ttype : FlattenTupleType(param->checked_type())) {
              auto name_hint = param->vid->name_hint;
              tvm::te::Tensor tensor = tvm::te::placeholder(//占位符，可以创建一个te变量
        ...
        Array<te::Tensor> VisitExpr_(const ConstantNode* op) final {
          ...
          void* data = op->data->data;//data 是一个 void* 类型的指针，指向常量数据的内存
          DataType dtype = DataType(op->data->dtype);
          if (op->is_scalar()) {//如果op是标量
            auto value = te::compute(
              ...
              else if (dtype == DataType::Int(8)) {//static_cast<const int8_t*>(data) 将 void* 指针转换为 const int8_t* 类型的指针
            ...
            VLOG(1) << "Constant tensor " << PrettyPrint(GetRef<Constant>(op));//打印多维常量张量的信息，查看是否有缺失
            VLOG(1) << "Generated tensor name: " << ss.str();//打印多维张量的名称
            constant_tensors_[op] = tensor;
            return { tensor };
            }
          }

        Array<te::Tensor> VisitExpr_(const CallNode* call_node) final { //这
          ...
          if (pattern_matcher_.find(op)) {
            if (pattern_matcher_.IsLeafOp(op)) {// Leaf op（叶子操作）：qnn.requantize（模式的最后一个操作）
              // Lower anchor op when pattern leaf op was reached  Anchor op（锚操作）：qnn.conv2d（模式中最重要和复杂的操作）
        ...
        Array<te::Tensor> VisitExpr_(const FunctionNode* op) final {
          LOG(FATAL) << "Primitive Functions can not contain nested functions.";//不允许嵌套函数
          }

        Array<te::Tensor> VisitExpr_(const LetNode* op) final {
          Array<te::Tensor> val = VisitExpr(op->value);
          ICHECK(!memo_.count(op->var));//如果已经操作过了
          memo_[op->var] = val;
        ...
        public:
        // Additional outputs
        Array<tvm::te::Tensor> fn_inputs_;// 函数输入的 TE 张量数组
        Array<te::Operation> scalars_;// 标量操作数组
        ...
        private:
        QnnPatternMatcher pattern_matcher_;//QNN 模式匹配器

        tvm::Target target_;
        std::ostringstream readable_name_stream_;
        // Cache device copy op for equivalence checking to reduce registry lookup
        // overhead(开销) for each invocation(调用) of call node when retrieving schedules.
      ...
      class ScheduleBuilder : public ExprVisitor {
        ...
        void VisitExpr_(const CallNode* call_node) final {// zeige
      ...
      TVM_REGISTER_GLOBAL("relay.backend.LowerToTE").set_body_typed([](Function prim_func) {
        ...
        // 使用 TVM 的日志系统
        LOG(INFO) << "Constant tensors map size: " << lower_te_compute.constant_tensors_.size();//zejia
        for (const auto& pair : lower_te_compute.constant_tensors_) {
          const ConstantNode* const_node = pair.first;
          const te::Tensor& tensor = pair.second;

          // 将 ConstantNode 转换为 Relay Constant 表达式进行打印
          Constant const_expr = GetRef<Constant>(const_node);
          LOG(INFO) << "Constant: " << PrettyPrint(const_expr);//zejia
          LOG(INFO) << "  Tensor's name: " << tensor->op->name << " shape=" << tensor->GetShape() << " dtype=" << tensor->GetDataType();//zejia
          }
