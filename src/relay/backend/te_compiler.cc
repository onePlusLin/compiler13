  ...
  namespace relay {
    ...
    namespace tec {
      ...
      class TECompilerImpl : public TECompilerNode {
        ...
        Array<tvm::runtime::Module> LowerExternalFunctions() {//奇怪在调试的过程中发现这里被调用的时候module已经是tir了
       ...
       * input/output sizes) 为输入函数创建函数元数据（例如计算输入/输出缓冲区大小）
      ...
      void UpdateFunctionMetadata(BaseFunc func,
        ...
        Optional<Map<GlobalVar, tir::PrimFunc>> prim_fns =              //prim_funcs: 与该函数相关的TIR原语函数映射
        ...
        Optional<GlobalVar> prim_fn_var = func->GetAttr<GlobalVar>("prim_fn_var"); // prim_fn_var: 函数的全局变量标识
        ...
        for (const auto& kv : prim_fns.value()) { //// 遍历原语函数并计算元数据,计算工作区大小、I/O大小和常量大小
          ...
          // Calculating size for I/O 这个函数计算所有输入输出缓冲区的总大小！
      ...
      IRModule LowerTE(const IRModule& module, const String& module_name, ProcessFn process_fn,
        ...
        Array<runtime::Module> new_external_mods = compiler->LowerExternalFunctions(); // zejia：调用外部编译器的作用是？
      ...
      Pass LowerTE(String module_name, CompilationConfig complilation_config, ProcessFn process_fn) {
        runtime::TypedPackedFunc<IRModule(IRModule, PassContext)> pass_func = [=](IRModule module,
          PassContext ctx) {
          return LowerTE(module, module_name, process_fn, complilation_config);// 这个是划分primitive和extenal函数的
          };

        return tvm::transform::Sequential( //zejia 
          { tvm::relay::transform::RelayToTIRTargetHook(complilation_config), // zejia
