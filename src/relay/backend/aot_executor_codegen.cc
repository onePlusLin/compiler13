    ...
    namespace backend {
      ...
      class AOTExecutorCodegen : public MixedModeVisitor {
        ...
        LoweredOutput Codegen(IRModule mod, relay::Function func, String mod_name) {
          ...
          //3. TE（Tensor Expression）降低
          IRModule lowered_mod =
            tec::LowerTE(mod_name, config_, [this, workspace_byte_alignment](BaseFunc func) {//config_ 是 All available targets
            ...
            // lowering process directly.  brief为输入函数创建函数元数据（例如计算输入/输出缓冲区大小）
