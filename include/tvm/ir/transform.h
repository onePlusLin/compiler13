...
#define TVM_REGISTER_PASS_CONFIG_OPTION(Key, ValueType)      \
  TVM_STR_CONCAT(TVM_PASS_CTX_CONFIG_VAR_DEF, __COUNTER__) = \
    ...
    /*!
     * \brief Managed reference class for PassInfoNode passinfonode的管理引用类
     * \sa PassInfoNode
     */
    class PassInfo : public ObjectRef {
      ...
      /*
      这是 TVM 框架的辅助宏，用于自动生成管理引用类的标准方法，包括：
      类型检查（确保引用的是 PassInfoNode 实例）。
      获取底层 PassInfoNode 指针的 operator->() 方法。
      引用计数管理（如 get()、reset() 等）
      */
      };
    ...
    class Pass : public ObjectRef {
      public:
      /*!
       * \brief Transform mod using the default PassContext in the current scope.z作用域
       *
       * \code
       *
