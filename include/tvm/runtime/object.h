...
上层代码（如 Python、TVM 编译模块）访问 Object 的安全接口
对象引用类：ObjectRef*/
class ObjectRef {
 public:
  /*! \brief default constructor */
...
//基类引用 ref，并返回一个子类引用 SubRef
template <typename SubRef, typename BaseRef>  //SubRef 是你想要转成的子类（例如 Function），BaseRef 是你当前持有的基类（例如 Expr）
inline SubRef Downcast(BaseRef ref) {         //向下转型： 用于在运行时验证类型转换的合法性
  if (ref.defined()) {                        //检查检查这个引用是否指向一个**非空（not null）**的对象
    ICHECK(ref->template IsInstance<typename SubRef::ContainerType>()) //检查 ref 实际指向的对象，是否真的是 SubRef::ContainerType 的一个实例
  ...
  return SubRef(std::move(ref.data_));//将baseRef的内部数据的指针所有权给subref，从而实现向下转型
}
