...
# raw_targets = [c -keys=arm_cpu,cpu -mcpu=cortex-m55]
def context(target, extra_files=None):
    ...
    """
    # 获取 TopHub 位置
    tophub_location = _get_tophub_location() # 用于确定从哪里获取 TopHub 预调优参数的 URL 位置
    if tophub_location == AUTOTVM_TOPHUB_NONE_LOC:
        return EmptyContext()
    
    # 创建一个 ApplyHistoryBest 调度上下文
    ...
    for tgt in targets:
        ...
        # 3) Target.kind
        possible_names = []
        device = tgt.attrs.get("device", "") # 在 TVM 的 Target 类中，device 是目标设备的一个属性，它表示实际运行代码的设备类型
        if device != "":
