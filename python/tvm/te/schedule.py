...
class Singleton(Object):
    """Singleton axis.""" # 标记“单元素轴”（如维度为1的轴）的元信息
...
class Schedule(Object):
    ...
    def normalize(self):
        ...
        规范化调度：插入必要的基准调整（如让循环变量从0开始），为后续边界推断和优化做准备。
        Returns
        -------
        sch : Schedule
    ...
    def create_group(self, outputs, inputs, include_inputs=False):
        ...
        将计算图中“输出到输入之间的操作”打包为一个“虚拟 Stage”（组），便于整体调度（如统一绑定硬件）
    ...
    def cache_read(self, tensor, scope, readers):
        ...
        为消费者创建输入张量的缓存，这个会改变消费者body，请在SPLIT/FUSE步骤之前调用。
    ...
    def cache_write(self, tensor, scope):
        """
        为输出张量创建“缓存副本”，先写入缓存，再按需同步到原始张量，优化写入效率。
        支持数据布局转换（如将中间结果按硬件友好的布局存储）。
...
# Stage 对应计算图中的一个 Operation,所有循环级优化的直接作用对象
@tvm._ffi.register_object
class Stage(Object):
...
# 支持根据张量维度、硬件参数等动态条件选择不同的调度策略，适配多场景下的性能优化。
@tvm._ffi.register_object
