...
# 该模块是TVM的FFI（Foreign Function Interface，外部函数接口）注册中心
# 核心作用：实现Python层与C/C++底层代码的交互，提供函数和对象的注册机制
# - 注册Python类与C层对象类型的映射，使Python能识别和操作C对象
# - 注册Python函数/C函数到TVM全局函数表，实现跨语言函数调用
# - 为TVM各模块提供统一的API初始化入口，将C层接口暴露到Python
"""
import sys
import ctypes
...
try:
    # 优先尝试导入Cython实现（性能更优，适用于编译时启用Cython的场景）
...
def register_object(type_key=None):
    ...
    def register(cls):
        ...
        # 如果已经在C++注册了，直接返回这个对象的内存地址
        if hasattr(cls, "_type_index"):
            tindex = cls._type_index
        else:
            # 创建了一个 C 语言的“无符号整型”(unsigned int) 变量，用来**“接收”** C++ 后端传回来的“类型索引号”。
            tidx = ctypes.c_uint()
            if not _RUNTIME_ONLY:
                ...
                if ret != 0:
                    return cls
            tindex = tidx.value # 从 ctypes 对象中取回 C++ 写入的那个值（例如 27），并存为一个 Python 整数。
...
def register_func(func_name, f=None, override=False):
    ...
    def register(myf):
        ...
        # 调用 C 函数 TVMFuncRegisterGlobal，完成注册
