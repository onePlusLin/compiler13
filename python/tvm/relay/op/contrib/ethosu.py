...
class BinaryElementwiseParams:
    """
    此类将解析对 ethosu.binary_elementwise 复合函数的调用
    并提取参数信息。
    ...
    def is_valid(self):
        """
        检查 BinaryElementwise 是否有合适的属性--数据类型dtype，形状shape，技术手册P102
...
class AddParams(BinaryElementwiseParams):
    ...
    def is_valid(self):
        ...
        """
        if not super().is_valid(): # z主要是检查数据类型和数据的shape类型
            return False
...
class MulParams(BinaryElementwiseParams):
    ...
    def is_valid(self):
        ...
        """
        if not super().is_valid():
            print("mul 复合函数的数据类型或形状不符合（或没有激活层）--无效，返回false\n"*3)
            return False
        # 检查下面的数据类型和形状
        if not check_valid_dtypes(
            [self.ifm, self.ifm2, self.ofm], supported_dtypes=[np.uint8, np.int8, np.int32]
        ):
            print("mul 函数的数据类型不支持，返回false\n"*3)
            return False
        print("mul函数支持NPU，返回true\n"*3)
        return True
...
def qnn_mul_pattern() -> tvm.relay.dataflow_pattern.DFPattern:
    ...
    # “匹配原来的 pattern（即单独的 qnn.mul） 或者 匹配一个 clip 算子，且这个 clip 的输入是原来的 pattern”
    pattern = pattern.optional(is_op("clip"))
    # qnn.mul(...) | clip(qnn.mul(...))
    return pattern
...
# 上面的是pattern的具体细节
# 将pattern注册到那个字典里 zejia注册
@register_pattern_table("ethos-u")
def pattern_table() -> List[Tuple[str, tvm.relay.dataflow_pattern.DFPattern, Callable]]:
    return [
        (
            ChannelPadParams.composite_name, # str : "ethos-u.channel-pad"
            pad_pattern(),
        ...
        (
            MulParams.composite_name,               # str : "ethos-u.mul"
            qnn_mul_pattern(),                      # mul_base  tvm.relay.dataflow_pattern.DFPattern qnn.mul(...) | clip(qnn.mul(...))
            lambda pat: MulParams(pat).is_valid(),  # function  检查传入的pat(call)的数据类型、形状等会，是否兼容，返回bool
        ),
        (
...
@requires_vela
def partition_for_ethosu(# 执行划分子图的python接口
    mod: tvm.ir.IRModule,
    ...
    **opts,
    ...
    if params:
        mod["main"] = bind_params_by_name(mod["main"], params) # 将权重参数绑定常量？方便常量折叠等优化

    pattern = relay.op.contrib.get_pattern_table("ethos-u")
    mod = relay.transform.InferType()(mod)
    mod = relay.transform.MergeComposite(pattern)(mod)  # 合并符合模式的子图composite，创建functions
    mod = relay.transform.AnnotateTarget("ethos-u")(mod)# 注释begin/end compiler_begin 和 compiler_end 
    mod = relay.transform.MergeCompilerRegions()(mod)   # 合并注释的子图:begin/end
    mod = relay.transform.InferType()(mod)              # 再次类型推导
    mod = relay.transform.PartitionGraph(mod_name)(mod) # 区域子图被lift为函数--tvmgen_default_ethos_u_main,常量绑定
    mod = relay.transform.InferType()(mod)
    print("改变输出前\n",mod)
    mod = preprocess.preprocess_ext_io()(mod)           # 合并输出，划分输出，将外部函数的输入输出为一个
    print("改变输出后\n",mod)
    return mod
