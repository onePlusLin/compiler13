...
def is_inline_non_compute_intensive_partitions_enabled() -> bool:
    ...
    """
    # 查询 Ethos-N 的编译设置，看看用户是否显式要求（或默认配置是否启用）
...
def partition_for_ethosn(mod, params=None, **opts):
    ...
    if params:
        # 将 Relay 图中的“变量”（placeholders for weights）替换为“常量”（Constant nodes）。这对后续的优化（如常量折叠）和 NPU 算子融合
        mod["main"] = bind_params_by_name(mod["main"], params)

    passes = [
        transform.InferType(),                      # 推断图中每个节点的数据类型和形状（shape）
        transform.FoldConstant(fold_qnn=True),
        transform.MergeComposite(pattern_table()),  # zejia 创造函数，这里可否修复？
        transform.AnnotateTarget("ethos-n"),        
        transform.MergeCompilerRegions(),
        transform.PartitionGraph(),
        ConvertEquivalents(),
    ]
    # 此 Pass 会“内联”那些计算量非常小的分区。如果一个 NPU 子图非常简单（例如只有一个 identity 或 reshape），将其卸载到 NPU 的开销（数据搬运、驱动调用）可能比直接在 CPU 上执行还要高。此 Pass 可能会将这些“不划算”的分区重新放回 CPU
    if is_inline_non_compute_intensive_partitions_enabled():
        passes.append(InlineNonComputeIntensivePartitions())

    # 将 passes 列表中的所有转换步骤打包成一个单一的、顺序执行的 Pass 流程
    # 在输入的 mod 上执行这个 Pass 流程
...
def pattern_table():
    ...
    def qnn_conv_pattern():
        # 1. 输入可以是 'nn.pad' 的输出，或者 'wildcard()' (任何东西)
        pattern = is_op("nn.pad")(wildcard(), wildcard()) | wildcard()
        
        # 2. 接着必须是一个 qnn.conv2d
        #    它的输入是上一步的 pattern，其余5个输入必须是常量 (权重, 尺度等)
        pattern = is_op("qnn.conv2d")(
            pattern, is_constant(), is_constant(), is_constant(), is_constant(), is_constant()
        )
        
        # 3. 接着必须是一个 nn.bias_add (输入是 conv 的结果和-个常量)
        pattern = is_op("nn.bias_add")(pattern, is_constant())
        
        # 4. 最后必须是一个 qnn.requantize (重新量化)
    ...
    def qnn_mul_pattern():
        ...
        """
        mul_op = is_op("qnn.mul") #  tvm.relay.dataflow_pattern.DFPattern 为什么这里定义了一个mul_op 是一个对象
        gen_mul_inputs = lambda x, y: mul_op( # 这个函数又返回一个mul_op对象，这个和上面的是一样的吗？
            x,              # (LHS tensor) - 待匹配的左输入
            y,
            is_constant(),  # lhs_scale
            is_constant(),  # lhs_zero_point
            is_constant(),
            is_constant(),
            is_constant(),
            is_constant(),
        ...
        input_is_right = gen_mul_inputs(is_constant(), wildcard())#要是两个都是is_constant（）那会返回什么？
        return input_is_left | input_is_right # 这个mul_op是返回一个bool吗，怎么判断是返回哪个？
