...
def is_op(op_name: str) -> "DFPattern":
    ...
    """
    op = get(op_name) # op = tvm.ir.Op.get(op_name)
...
class DFPatternCallback:
    """A Callback for Pattern Rewriting.什么是callback？回调函数吗
    ...
    def rewrite(self, expr: Expr) -> Expr:
        ...
        """
        return rewrite(self, expr) # 调用的是全局的函数
    ...
    def callback(self, pre: Expr, post: Expr, node_map: tvm.ir.container.Map) -> Expr:
        ...
        node_map : tvm.ir.container.Map[DFPattern, List[Expr]] 这又是什么对象？
...
# _ 表示内部使用的类，外部用户不应该直接依赖或调用它。
class _DFPatternCallback(Object): 
    """
    将DFPatternCallback 规则，转换为 TVM C++ 核心引擎能够理解和执行的底层对象
    C++ implemenation"""

    def __init__(self, pattern, callback, require_type, rewrite_once):
        self.__init_handle_by_constructor__( # 调用一个 C++ 对象的构造函数来初始化自己
...
def rewrite(callbacks, expr: Expr, mod: Optional[_ir.IRModule] = None) -> Expr:
    ...
    result : tvm.relay.Expr
        The Expression with matched subgraphs rewritten by the callbacks.
    """
    print("ZEdebug:rewrite:使用的模式是:",str(callbacks),"\n,callback 的类型：",type(callbacks))
    
    if mod is None:
        mod = _ir.IRModule()
    callbacks = [callbacks] if isinstance(callbacks, DFPatternCallback) else callbacks
    print(len(callbacks),"callbacks的长度！\n")
    
    tmp = []
    from tvm.relay.backend.contrib.ethosu.legalize import MulRewriter
    for callback in callbacks:
        # print("callback.params_class:",callback.params_class)
        if isinstance(callback,MulRewriter):
            print("rewrite HERE: 检测到 MulRewriter 被调用！")
        
        print("callback 全部属性：",vars(callback))
        assert callback.pattern is not None
        tmp.append(
            _DFPatternCallback(
                # 这里将对象的方法导入，即算子的重写匹配规则、callback规则，后面两个是继承的一般没定义
