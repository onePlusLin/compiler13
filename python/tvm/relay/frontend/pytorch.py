...
# 将 PyTorch 模型转换为 TVM Relay 中间表示的核心函数
def from_pytorch(
    script_module,
    input_infos,
    ...
    # 创建TVM IR模块(中间表示)和基础工具类Prelude
    mod = tvm.IRModule()
    prelude = Prelude(mod)
    # 标记是否启用lower_all_tuples优化(将元组操作展开)
    enable_lower_all_tuples = True

    # 初始化PyTorch操作到Relay操作的转换器
    converter = PyTorchOpConverter(prelude, default_dtype, use_parser_friendly_name)

    # 复制PyTorch计算图，避免修改原始图
    graph = script_module.graph.copy()

    # 检查输入是否包含元组或列表类型，决定是否启用lower_all_tuples优化
    graph_inputs = list(graph.inputs())
