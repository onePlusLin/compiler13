...
class OperatorConverter(object):
    ...
    def if_not_used(self, op):
        input_tensors = self.get_input_tensors(op)
        for n in self.not_used:
    ...
    def convert_op_to_relay(self):
        ...
        for op_idx in range(self.subgraph.OperatorsLength()):
            ...
            if len(in_names):
                input_tensors = self.get_input_tensors(op)
                for i in input_tensors:
                    tensor_idx = i.tensor_idx
                    name = get_tensor_name(self.subgraph, tensor_idx)
                    tensor = i.tensor
                    if name in in_names:
                        skip_flag = False
                        model_input_name = name
                        input_shape = tuple(tensor.ShapeAsNumpy())
                        tensor_type = tensor.Type()
                        shape = input_shape
                        dtype = _decode_type(tensor_type)
                        input_var = set_span(
                            _expr.var(model_input_name, shape=shape, dtype=dtype), model_input_name
                        )
                        self.exp_tab.set_expr(model_input_name, input_var)
                        break

                if skip_flag:
                    continue

            if self.if_not_used(op):
                for tensor in output_tensors:
                    self.not_used.append(tensor) 
                continue
            ...
            for tensor in output_tensors:
                name = get_tensor_name(self.subgraph, tensor.tensor_idx)
...
def from_tflite(model, shape_dict=None, dtype_dict=None, op_converter=OperatorConverter):
    ...
    if len(out_names):
        outputs = [exp_tab.get_expr(i) for i in out_names]
    else:
