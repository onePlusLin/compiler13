...
def copy_constants():
    ...
    def _planner(cached_func, const_dict, sch):
        ...
        def _is_matmul(tensor):
            ...
            return a.shape[1:3] == [1, 1] and b.shape[1:3] == [1, 1] # 全连接层

        def _visit(tensor, reader, lut):
        # reader: 读取当前张量的张量（即当前张量的消费者） 和 lut: 查找表（LookUp Table），一种特殊类型的常量
            if tensor not in planned:
                planned.add(tensor)
                print("当前tensor is :\n",tensor)
                if isinstance(tensor.op, tvm.te.PlaceholderOp) and tensor != lut:  # 输入参数或常量?
                    ...
                    print("\is_same is :",is_same)
                    print("\tindex is :",index)
                    # Along with constants, also skip for FullyConnected to corresponds
                    # with Vela behavior
                    if index in const_dict and not _is_matmul(reader):
                        print("\t被复制的张量是:",tensor,"\treader is :",reader)
                        sch.cache_read(tensor, "global", [reader])  # 输入是不会的

                elif isinstance(tensor.op, tvm.te.ComputeOp):
                    if "lut" in tensor.op.attrs.keys():
                        lut = tensor.op.attrs["lut"]
                    for input_tensor in tensor.op.input_tensors:
                        print("\tinput_tensor is :",input_tensor,"\ttensor is :",tensor,"\tlut is :",lut)
