...
def lower_ethosu(sch, args, const_dict, name="main"):
    ...
    with tvm.transform.PassContext(config=curr_cfg):
        print("before schedule_to_module,sch:\n",sch)
        print("before schedule_to_module,args:\n",args)
        print("before schedule_to_module,name:\n",name)
        mod = schedule_to_module(sch, args, name)       # 将调度转换为 TIR 模块
        print("after schedule_to_module,mod:\n",mod)

        mod = tvm.tir.transform.Simplify()(mod)
        print("after Simplify,mod:\n",mod)
        
        mod = ethosu_passes.RemoveConcatenates()(mod)
        print("after RemoveConcatenates,mod:\n",mod)
        
        mod = tvm.tir.transform.InjectRollingBuffer()(mod)
        print("after InjectRollingBuffer,mod:\n",mod)
        
        mod = tvm.tir.transform.StorageFlatten(64)(mod)
        print("after StorageFlatten,mod:\n",mod)
        
        mod = tvm.tir.transform.UnrollLoop()(mod)
        print("after UnrollLoop,mod:\n",mod)
        
        mod = tvm.tir.transform.Simplify()(mod)
        print("after Simplify,mod:\n",mod)
        
        mod = tvm.tir.transform.LoopPartition()(mod)
        print("after LoopPartition,mod:\n",mod)
        
        mod = ethosu_passes.RemoveZeroStores()(mod)
        print("after RemoveZeroStores,mod:\n",mod)
        
        mod = tvm.tir.transform.Simplify()(mod)
        print("after Simplify,mod:\n",mod)
        
        mod = tvm.tir.transform.RemoveNoOp()(mod)
        print("after RemoveNoOp,mod:\n",mod)
        
        mod = ethosu_passes.ReplaceOperators()(mod)
        print("after ReplaceOperators,mod:\n",mod)
        
        mod = tvm.tir.transform.RemoveNoOp()(mod)
        print("after ReplaceOperators,mod:\n",mod)
        
        mod, const_dict = ethosu_passes.EncodeConstants(const_dict)(mod)
        print("after EncodeConstants,mod:\n",mod)
        print("after EncodeConstants,const_dict:\n",const_dict)
        
        mod = ethosu_passes.HoistAllocates()(mod)
        print("after HoistAllocates,mod:\n",mod)
        
        mod = tvm.tir.transform.RemoveNoOp()(mod)
        print("after RemoveNoOp,mod:\n",mod)
        
        print("------------------------MergeConstants------------------------")
        print("mod:\n", mod)
        print("const_dict:\n", const_dict)
        mod, const_dict = ethosu_passes.MergeConstants(const_dict)(mod)
        print("after MergeConstants")
        print("mod:\n", mod)
        print("const_dict:\n", const_dict,"\n------------------------End MergeConstants------------------------")
...
def extract_constants(func):
    ...
    const_dict : dict of int to numpy.ndarray
        A dict of the extracted constants keyed by their param index.不理解这句话，params一般是1.consts其实就是按顺序排的
...
def _lower_to_tir(func, cascader=None):
    ...
    """
    print("zedebug:***************** _lower_to_tir: *****************")
    print("zedebug: extract_constants before,func:\n", func)
    # 这里是将函数里面的运算常量提取出来到consts,按照排序
    func, consts = extract_constants(func)
    # 分离出的常量和调度后的常量是什么关系，可能就是在这里分离常量的时候和调度的时候漏掉了一些？
    print("zedebug: extract_constants after,func:\n", func)
    print("zedebug: extract_constants after,consts:\n", consts)
    
    mod = tvm.IRModule.from_expr(func)
    func = relay.transform.InferType()(mod)["main"]
    
    print("zedebug:lower_to_te befor  func :\n", func)
    cached_func = lower_to_te(func)
    print("zedebug:lower_to_te after  cached_func:\n", cached_func)# 被包装了
    print(cached_func.funcs.astext())
    
    print("cached_func.outputs:\n",cached_func.outputs)
    print("cached_func.inputs:\n",cached_func.inputs)
    # print("cached_func.constant_tensors's size:\n",cached_func.constant_tensors.size())
    # print("cached_func.shape_func_param_states's size:\n",cached_func.shape_func_param_states.size())

    
    
    s = schedule(cached_func, consts, cascader)
    print("zedebug:lower_to_tir:schedule:s:\n", s)
    print("zedebug:lower_to_tir:cached_func:\n", cached_func)
    # print("zedebug:lower_to_tir:consts:\n", consts)一样的
    
    
    mod, consts = lower_ethosu(s, cached_func, consts)
    print("zedebug:lower_ethosu after:mod:\n", mod)
    print("zedebug:lower_ethosu after:consts:\n", consts)
    
    print("zedebug:***************** _lower_to_tir end *****************")
