...
class Conv2DRewriter(DFPatternCallback):
    """
    将conv2d相关的复合函数转换为ethosu_conv2d运算符
    Convert conv2d related composite functions into ethosu_conv2d operators"""

    def __init__(self):
        super().__init__(require_type=True)
        self.pattern = (wildcard().has_attr({"Composite": "ethos-u.qnn_conv2d"}))(wildcard())# wildcard（）是什么意思？

    def callback( self, pre: tvm.relay.Expr, post: tvm.relay.Expr, node_map: tvm.ir.container.Map ) -> tvm.relay.Expr:
        
        print("ZEdebug:Conv2DRewriter -重写前的",pre)
        print("ZEdebug:Conv2DRewriter -重写后的",post)
        # 这个类可以解析一个Call为一个ethos_u.qnn_conv2d复合函数，并提取相关的量化参数,这里为什么传入替换后的expr
        # 因为post才是最新的
        params = ethosu_patterns.QnnConv2DParams(post.op.body)
        # 输入特征图 (IFM) 是作为参数传给复合函数的，所以从 post.args[0] 获取
        params.ifm.tensor = post.args[0]
        channels_map = {
            "NHWC": 3,
        }
        ...
        if params.activation:
            ...
            clip_max = 0
            '''
            pack_biases 函数（由 Arm 的 Vela 编译器工具提供）会将32位的浮点 bias、ifm scale、weight scales 等参数，
            通过一系列复杂的数学运算，打包成一个硬件可以直接使用的、由8位无符号整数组成的数组。
            '''
...
class BinaryElementwiseRewriter(DFPatternCallback):
    ...
    def callback(
        ...
        '''
        pre:代表在 rewrite 引擎开始处理当前匹配区域之前，从原始计算图中切出的、与 pattern 完全匹配的子图的快照，主要用于对比和调试
        post:代表与 pattern 匹配的子图，但其所有输入（子节点）都已经经过了 rewrite 引擎的递归访问和可能的重写
        node_map：一个字典。它建立了从你在 self.pattern 中定义的“占位符”（如 wildcard() 对象）到 post 子图中实际匹配到的具体节点的映射
        '''
        params = self.params_class(post.op.body) #  'params_class': <class 'tvm.relay.op.contrib.ethosu.MulParams'>
        ...
        # 5. 构建新的底层算子
        # 调用 ethosu_ops.ethosu_binary_elementwise 的构造函数来创建一个新的 CallNode。
...
class AddRewriter(BinaryElementwiseRewriter):
    def __init__(self):
        ...
        print("zedebug:AddRewriter:params_class",ethosu_patterns.AddParams)
        print("zedebug: ethosu_patterns.MulParams.composite_name",  ethosu_patterns.AddParams.composite_name) # ethos-u.mul
...
class SubRewriter(BinaryElementwiseRewriter):
    def __init__(self):
        ...
        print("zedebug:SubRewriter:params_class",ethosu_patterns.SubParams)
        print("zedebug: ethosu_patterns.MulParams.composite_name",  ethosu_patterns.SubParams.composite_name) # ethos-u.mul
...
class MulRewriter(BinaryElementwiseRewriter):
    def __init__(self):
        ...
        # print("zedebug:MulRewriter:params_class",ethosu_patterns.MulParams) # <class 'tvm.relay.op.contrib.ethosu.MulParams'>
        # print("zedebug: ethosu_patterns.MulParams.composite_name",  ethosu_patterns.MulParams.composite_name) # ethos-u.mul
        # print("pattern:",(wildcard().has_attr({"Composite": ethosu_patterns.MulParams.composite_name}))( # 
        #         wildcard(), wildcard()
        #     ),)

# zejia 添加一个只有一个输入的mul匹配规则
class myMulRewriter(BinaryElementwiseRewriter):
    def __init__(self):
        super().__init__(
            params_class=ethosu_patterns.MulParams,
            pattern=(wildcard().has_attr({"Composite": ethosu_patterns.MulParams.composite_name})(wildcard()))
        )
        print("这是只有一个输入的mul，它的输入是两个一样的变量！")
        print("pattern:",(wildcard().has_attr({"Composite": ethosu_patterns.MulParams.composite_name}))(wildcard()))
...
# zejia0
@util.create_npu_function_pass(opt_level=1) # LegalizeEthosU 类被装饰器包装，变成了标准的 TVM Pass
class LegalizeEthosU:
    ...
    def transform_npu_function(self, _, func: relay.Function) -> relay.Function:# 这里写的传入的是function类型，传出也是
        """
        将复合的NPU算子改名
        This is the method that replaces the operations with hardware/codegen supported
        operations.
        """
        print("--------------LegalizeEthosU transform_npu_function start-------------")
        # 先初始化重写算子的规则
        rewriters = [
            PartitionedSplitRewriter(), # {"Composite": ethosu_patterns.SplitParams.composite_name}
            SplitRewriter(),            # is_op("split")(self.split_in)
            ChannelPadRewriter(),       # {"Composite": ethosu_patterns.ChannelPadParams.composite_name}
            Conv2DRewriter(),
            ...
            AddRewriter(),
            SubRewriter(),
            MulRewriter(),          # {"Composite": ethosu_patterns.MulParams.composite_name}!!!!!!!!这个需要注意！！
            myMulRewriter(),        # one input params mul
            MinRewriter(),
            MaxRewriter(),
            ShlRewriter(),
            AbsRewriter(),
        ...
        for rewriter in rewriters:
            print("zedebug:rewrite start --初始化重写规则结束:",str(rewrite))
            
            if(isinstance(rewrite ,MulRewriter)):
                print("mul 开始")
            
            # 调用rewrite函数进行重写
            # <class 'tvm.relay.function.Function'> 在装饰器里，将mod的npu function取出来后调用的这个transform_npu_function函数
            # func = rewrite(rewriter, func) #rewrite(callbacks, expr: Expr, mod: Optional[_ir.IRModule] = None) -> Expr:
            print_func = rewrite(rewriter, func)
            
            
            print("重写之前的表达式是：",func)
            print("重写之后的表达式是：",print_func)
            print("-------------------一次重写 结束----------------")
            func = print_func
