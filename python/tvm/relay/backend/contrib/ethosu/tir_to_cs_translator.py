...
class AcceleratorArchConfig:
    def __init__(self, total_shram_banks):
        # 每个SHRAM存储体的大小为1024字节
        self.shram_bank_size = 1024
        # SHRAM存储体的总数量，这个值根据不同型号的Ethos-U加速器而不同
...
def analyze_scratch_memory_acesses(mod: tvm.IRModule, candidate_regions_for_scratch: List[int]):
    ...
    该函数分析IRModule中的中间张量，这些张量可能是由池变量的偏移量（通过Let节点）和/或 allocate节点 产生的。
    分配节点将被折叠成一个带有偏移量的TVMBackendallocWorkspace调用。
    最终，这将生成一个从每个这样的节点到一个名为元组的RegionOffset的映射，该元组包含该区域和获得的偏移量
    ，如上所述。
    Parameters
    ----------
    mod: tvm.IRModule
    ...
    tvm_backend_alloc_workspace_size : int
        ...
        服务所需的 tvm_backend_alloc_workspace 调用的大小
        剩余分配节点（如果有）
    ...
    # print("ZEdebug:analyze_scratch_memory_acesses-codegen.py,!! !!! mod:\n",mod)
    # print("ZEdebug:analyze_scratch_memory_acesses-codegen.py,!! !!! primfunc:\n",primfunc)
    # print("ZEdebug:analyze_scratch_memory_acesses-codegen.py,!! !!! primfunc.body:\n",primfunc.body)
    
    # "pool_args": [I.AllocatedPoolInfo(pool_info=metadata["ir.WorkspacePoolInfo"][0], allocated_size=45936)],
    # 会申请一个scratch_region，并在里面申请一些内存池，大小参数是pool_param，用于存放
    if "pool_args" in primfunc.attrs.keys():
        pool_args = primfunc.attrs["pool_args"]
        for pool_arg in pool_args:
            
            # pool_arg是一个参数，里面有内存池的参数，内存池的参数是一个AllocatedPoolInfo对象，里面有内存池的参数和分配的大小
            # 内存池变量
            pool_param = primfunc.params[int(pool_arg.pool_var_idx)]
            
            # 2. 分配一个专属区域
            pool_var_region_map[pool_param] = candidate_regions_for_scratch.pop()# 【5，2，1】
            
            # 3. 在主映射表中记录这个内存池
            scratch_region_map[pool_param] = RegionOffset(
                region=pool_var_region_map[pool_param], offset=None
            )
    # 讲letstmt的内存地址记录下来，最后是 ethosu_write_let: RegionOffset(region=1, offset=64)...
    ...
    # 会遍历 primfunc.body 中的每一个 TIR 语句节点，并对每个节点调用 analyze_pool_access 函数
...
# 根据buffer_type 或var 获取 region，buffer类型的是static_regions，var是scratch_region_map
def _get_region(buffer_type, var=None, scratch_region_map=None):
    ...
    static_regions = {
        ...
        BufferType.input: 3,
        BufferType.output: 4,
        BufferType.shram: int((1 << 8) | (3 << 0)),# 一种编码格式，看不懂
    }
...
def translate(tir_module, params):
    ...
    # 会将pool_args中的pool_var_region_map，和let节点、allocate节点映射到对应的区域
    (
        scratch_region_map,
        dynamic_allocation_size,
        dynamic_allocation_region,
    ...
    # 提取ethosu constants变量，还有params,以及local allocate语句，包括常量值、形状、数据类型和缓冲区类型
    buffer_info = extract_buffer_info(tir_module, params)
    
    # 从TIR模块中提取所有的Ethos-U外部调用,call_extern函数是用来干啥的？
    call_extern_list = extract_call_extern_list(tir_module)
    
    # 为上面的call_extern，调用C++底层的函数，将这些外部函数翻译为ethosu操作--一个包含对应的算子参数的vela object
    _npu_ops = list()
    for call_extern in call_extern_list:
        _npu_ops.append(translate_ethosu_tir_call_extern(call_extern))
        
        # daying 调试信息
    print("----------------zedebug:translate------------------")
    print("\ntir_module:\n",tir_module)
    print("\nbuffer_info:\n",buffer_info.keys())
    print("\nscratch_region_map:\n",scratch_region_map)
        
    # 为缓冲区分配地址，并处理常量数据的编码，这里出现bug
    ...
    # 使用Ethos-U Vela API生成寄存器命令流并创建驱动载荷
...
def extract_buffer_info(
    ...
    # 遍历"ethos-u.constants"里面的常数，将这些常数的数据、形状、类型、BufferType.constant一直是1，记录到buffer_info中；
    ...
    # 这个pool_args到底是什么？pool_var_idx属性又是什么？
    if "pool_args" in primfunc.attrs.keys():
        pool_args = primfunc.attrs["pool_args"] 
        pool_param_indices = [allocated_pool_info.pool_var_idx for allocated_pool_info in pool_args] # 2

    # 遍历PrimFunc的参数：

    # 如果参数尚未在缓冲区信息中，则根据其索引确定类型：
    # 如果是池参数索引，标记为scratch类型（临时存储）
    # 否则标记为input_or_output类型（输入或输出）
    ...
    # print("zedebug: extract_buffer_info-tir_to_cs_translator!!! buffer_infor!!!:\n",buffer_info)
    return buffer_info
...
def assign_addresses(buffer_info, npu_ops, scratch_region_map):
    ...
    ----------
    buffer_info : dict
        常量、local、输入输出的缓冲区信息、也有一个内存池变量、和一个共享内存的变量
    ...
    """
    # 替换 NpuFeatureMap 对象（用于 IFM/OFM）中的逻辑地址。
    def replace_npu_fm_with_address(npu_fm):
        ...
        # 如果这个 buffer 在 scratch_region_map（暂存区地址簿）中，就从那里获取 address (偏移) 和 region (区域)
        if buffer in scratch_region_map.keys():
            address = scratch_region_map[buffer].offset
            region = scratch_region_map[buffer].region
        
        # 在buffer_info中，获取 buffer 的 address 和 buffer_type
        else:
        ...
        # 计算由 BufferLoad 的 indices 提供的额外字节偏移量，int8这里index==1，int16这里index==2，int32这里index==4?
        index = npu_fm.tiles.addresses[0].indices[0] * (
            np.iinfo(np.dtype(npu_fm.tiles.addresses[0])).bits // 8
        )
        # NpuFeatureMap 的地址字段（原本是一个 BufferLoad 对象）突变 (mutate) 为一个纯整数（物理地址 = 基地址 + 索引偏移）
        ...
        return npu_fm
    # 用于 vapi.NpuAddressRange 对象 (内存范围, 即 Weights/Biases/DMA
    def replace_npu_address_range_with_address(npu_addr_range):
        ...
        assert buffer in buffer_addresses.keys(), f"searching for buffer : {buffer}, but not found" #什么样的原因会导致找不到这个地址呢？
        # print("zedbug: replace_npu_address_range_with_address!!! npu_addr_range!!!:\n",npu_addr_range)
        address, buffer_type = buffer_addresses[buffer]
    ...
    def replace_tir_loads(npu_object):
        ...
        if isinstance(npu_object, vapi.NpuAddressRange):
            print("-------------debug start---------------")
            print(npu_object.address)
            buffer = npu_object.address.buffer.data
            if buffer not in scratch_region_map.keys():
                if buffer not in buffer_addresses.keys() :
                    print("找不到这个的地址：:\n",npu_object.address)
                else: 
                    print("buffer:\n",buffer)
                    print("buffer's buffer_address:\n",buffer_addresses[buffer])
    ...
    # 初始化一个列表，用于收集所有常量（权重/偏置）的十六进制字符串
    # 一个偏移累加器（“水位线”）。用于在单一的常量缓冲区中堆叠多个常量张量
    # 初始化第二个“地址簿”。这个字典将存储所有非暂存区 (non-scratch) 缓冲区的地址
    constant_hex_data = []
    ...
    for _buffer, info in buffer_info.items():
        ...
        if info.values is not None:
            ...
            dtype_bytes = np.iinfo(np.dtype(info.dtype)).bits // 8 # 将一个数据转换为字节数
            size_in_bytes = dtype_bytes * np.prod(list(info.shape))# 计算这个常量所占的字节数
            # Every memory address the NPU access have to be 16 byte aligned
            size_in_bytes = util.round_up(size_in_bytes, 16)
            constant_tensor = np.resize(info.values, size_in_bytes // dtype_bytes)
            constant_tensor = constant_tensor.tobytes().hex()       # 16进制对齐
            constant_hex_data.append(constant_tensor)               # 这是对齐的位置，没一个常量存储开始的位置
            total_constant_len += len(constant_tensor) // 2         # 推进“水位线”。下一个常量将被放置在这个常量之后
        # 处理 IO
        else:
            if info.btype == BufferType.input_or_output or info.btype == BufferType.input:
                buffer_type = info.btype
                # 如果类型不明确，不知道是输入还是输出，则根据输入输出的buffer_var进行分类
            ...
            # 如果是共享区的
                ...
                # local 内存（SHRAM）的缓冲区不从 0 开始。它们从一个固定的、预定义的地址（NPU 内部 SRAM 的查找表起始地址）开始。
    ...
    for npu_op in npu_ops:
        for attr_name, attr in npu_op.__dict__.items():
            if isinstance(attr, vapi.NpuAddressRange):
                if attr.address.buffer.data == "p2_encoded":
                    print("npu_op.__dict__")
                    print("zedbug: p2_encoded!!!:\n",attr)
...
def translate_ethosu_tir_call_extern(tir_call_extern):
    ...
    first argument
    根据extern call的第一个参数,（op的name）来调用对于的翻译函数，将这些call externs翻译为包含对于算子参数的vela object
    """
...
def translate_ethosu_copy(tir_call_extern: tvm.tir.Call) -> vapi.NpuDmaOperation:
    ...
    将NPU Relay生成的TIR编译过程中的TIR外部调用翻译成Ethos-U操作
