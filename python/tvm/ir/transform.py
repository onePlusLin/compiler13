...
# 该模块定义TVM中跨所有IR（中间表示）变体的通用Pass（优化/分析步骤）基础设施，
# 核心作用：提供统一的接口和工具，用于定义、配置、执行和组合IR优化/分析步骤，
# 支撑TVM编译流程中对IRModule（中间表示模块）的各类处理（如算子融合、常量折叠、类型检查等）。
"""
import inspect
import functools
...
class PassInfo(tvm.runtime.Object):
    """
    Pass的元数据容器，存储单个Pass的核心信息，用于Pass管理和依赖解析。
...
class PassContext(tvm.runtime.Object):
    """
    Pass执行的上下文环境，管理Pass运行时的全局配置和状态。
    The basis where a Relay optimization/analysis runs on.
    
    Each pass context contains a number of auxiliary辅助 information that is used
    ...
    opt_level : Optional[int]
        全局优化级别（0-3），低于此级别的Pass会被跳过，0等级为跳过pass优化
        The optimization level of this pass.
...
class Pass(tvm.runtime.Object):
    """所有Pass的基类，定义Pass的统一接口。

    无论是优化Pass（如算子融合）还是分析Pass（如类型检查），都需继承此类，
    后端通过统一接口调用Pass，屏蔽不同Pass的实现差异。
    """
    ...
    def __call__(self, mod):
        """执行Pass，输入IRModule，返回优化/分析后的IRModule"""
...
def module_pass(pass_func=None, opt_level=None, name=None, required=None):
    ...
    当提供pass_func时，此函数返回一个回调。
    否则，它将起到装饰器的作用。
    This function returns a callback when pass_func is provided.
    Otherwise, it serves a decorator function.

    Pass_func也可以是带有transform_module方法的类类型。
    这个函数将使用transform_module创建一个修饰过的ModulePass
    作为传递函数。
    ...
    # 如果没有传入pass_func,会被装饰为一个ModulePass对象
