...
TVMC 工具的编译模块，负责将输入模型（如ONNX、TensorFlow等）编译为TVM可执行模块，
支持命令行参数解析、模型转换、优化编译、结果导出等全流程功能。
...
# 告诉 Pylint（Python 代码静态分析工具）忽略 "invalid-name" 规则检查
# 创建一个名为 "TVMC" 的日志记录器实例
...
def drive_compile(args):
    """
    在调用编译前--compile_model，转换输入模型为tvmc model对象，并处理参数
    Invoke调用 tvmc.compiler module with command line arguments
...
#   调用前端加载函数，被转换为relay，最后输出一个 TVMCModel 对象，封装了mod 和 params
    tvmc_model = frontends.load_model(args.FILE, args.model_format, args.input_shapes)
    print("最最最开始的模型：",tvmc_model.mod)
    print("导出一个relay 模型")
    # with open("/home/linzejia/app/tvm/apache-tvm-src-v0.13.0/aaaaaaaa/myrelaymod/yolov5int8nometadata.relay", "w") as f:
    #     f.write(tvmc_model.mod.astext(show_meta_data=True))
    tvmc_model.save("/home/linzejia/app/tvm/apache-tvm-src-v0.13.0/aaaaaaaa/myrelaymod/yolov5.tar")

#   
    dump_code = [x.strip() for x in args.dump_code.split(",")] if args.dump_code else None

#   解析 --dump-offloads 参数，参数用于显示模型中各个操作被分配到哪个后端执行的信息
    dump_offloads = args.dump_offloads if args.dump_offloads else ""

    additional_targets = reconstruct_target_args(args)
    # 通过 target_from_cli 解析主目标设备（args.target）和额外目标
    workspace_pools_target, extra_targets = target_from_cli(args.target, additional_targets)
    # 通过 parse_graph_transform_args(args) 解析图形转换相关参数（如布局优化、混合精度等）字典
    ...
    compile_model(
        ...
        # ** 操作会将字典中的每个键值对拆解为 key=value 的形式，作为关键字参数传递给 compile_model 函数
        **transform_args,
    )
...
def compile_model( #编译model的开端
    ...
    """
    负责将加载的模型（TVMCModel）转换为可在目标设备上运行的 TVM 模块，支持优化、调优记录应用、代码导出等功能
    可以被图形执行器执行
    ...
    tuning_records : str
        调优记录文件路径（由 tvmc.tune 生成），用于加载预优化的 kernel 配置，提升性能
    ...
    # 若需要导出 TIR 代码（"tir" in dump_code），调用 add_tir_to_dumps 添加一个调试 Pass，用于收集 TIR 函数。
    if "tir" in dump_code:
        config, dumps = add_tir_to_dumps(config, dumps)

    initial_relay = None
    if dump_offloads != "":
        # add suffixes后缀 to the span field for calls in Relay
    ...
    for codegen_from_cli in extra_targets:
        ...
        if codegen["config_key"] is not None:
            ...
            # 添加codegen配置:relay.ext.ethos-u.options:{'accelerator_config': 'ethos-u55-256'}
            # relay.ext.cmsisnn.options:{'mcpu': 'cortex-m55'}
            
    # 配置 PassContext，执行pass优化
    with tvm.transform.PassContext(
        ...
        # 解析传入的图形转换参数选项,locals()包含当前作用域中的所有局部变量及其对应的值
        transform_args = parse_graph_transform_args(locals())   # locals() 返回当前作用域中的局部变量的字典表示。
        # 应用图形转换
        mod = apply_graph_transforms(mod, transform_args)
        

        # 划分子图，应该包括ethosu和cmsis-nn
        for partition_function, opts in zip(partition_functions, partition_opts):
            mod = partition_function(mod, params, mod_name=mod_name, **opts) # 划分子图，找到可以offload的全部子图
            print("ZEdebug:划分子图后的mod:\n",mod)

        # 分配算子到不同的target设备,none
        if initial_relay:
            # dump which operations are offloaded to which backend
            dump_operation_offloads(mod, initial_relay, dump_offloads)

        # 分不同的情况执行autotvm或fallback/tophub优化，并编译
        ...
        print("ZEdebug:compiler_model模型已经build完成！") # yolov5没有跑到这里
        # print("mod :",graph_module) 这里输出的是factory_module : tvm.relay.backend.executor_factory.ExecutorFactoryModule
        #    The runtime factory for the TVM graph executor.

        # Generate output dump files with sources
        # 输出中间的代码文件，根据dump_code参数
...
def build(
    ...
    print("ZEdebug:func-build mod_name:",mod_name )
    """
    ...
    use_vm: bool
        使用 TVM 虚拟机（Virtual Machine，VM）编译 Relay 模型
        Whether to use the VM to compile the model as opposed to the graph executor
    mod_name: str
        The module name
