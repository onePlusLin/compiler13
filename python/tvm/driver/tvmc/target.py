...
# additional_target_options = {'c': {'mcpu': 'cortex-m55'}, 'cmsis-nn': {'mcpu': 'cortex-m55'}, 'ethos-u': {'accelerator_config': 'ethos-u65-256'}}
def target_from_cli(target, additional_target_options=None):
    """
    这是处理用户cli输入的target字符串的函数，主要为将其转换为target对象，还有一些额外target的处理。
    ...
    if os.path.isfile(target):
        ...
        tvm_targets = [
            ...
            # t = {'name': 'ethos-u', 'opts': {}, 'raw': 'ethos-u', 'is_tvm_target': False}
            # {'name': 'c', 'opts': {}, 'raw': 'c', 'is_tvm_target': True}
